...
import com.fahimdev.shared.network.ApiResult as SharedApiResult
    ...
    suspend fun <T : Any?> apiRequest(call: suspend () -> Response<T>): ApiResult<T?> {
        return try {
            ...
            if (response.isSuccessful &&
                ...
                SharedApiResult.Success(response.body())
            } else {
                ...
                SharedApiResult.Error(message.toString(), response.code())
            }
...
//            }
            SharedApiResult.NetworkError

        } catch (e: Exception) {
//            if (BuildConfig.DEBUG) {
//                Log.d("Network Error", "Unexpected Error: ${e.localizedMessage}")
//            }
            SharedApiResult.Error(e.localizedMessage ?: "Unknown Error")
        }
    }
}